...
require 'cabin'
require 'log-courier/iomultiplex'
require 'multi_json'
require 'openssl'
require 'socket'
require 'thread'
...
module LogCourier
  ...
  class TCPTransport
    ...
    def initialize(options = {})
      ...
      # Cannot use :raw as there's a bug in JrJackson (https://github.com/guyboertje/jrjackson/issues/46)
      @json_options = { raw: @options[:raw_events] }
      unless @options[:raw_events]
        @logger.warn 'Disabling MultiJSON raw processing due to Logstash ' \
          'version (see https://github.com/driskell/log-courier/issues/247)'
      end
    rescue => e
    ...
    def process_jdat(nonce, data, client, event_queue)
      ...
      loop do
        ...
        if ret.nil?
          ...
          @logger.warn 'JDAT length extraction failed', id: client.id, ret: ret, length_length: length_buf.bytesize
          client.shutdown
          return
        end
        ...
        if ret.nil? or data_buf.bytesize < length
          @logger.warn 'JDAT data extraction failed', id: client.id, ret: ret, data_length: data_buf.bytesize
          client.shutdown
          return
        end
  ...
  class TCPClient < IOMultiplex::SSLUpgradingIO
    ...
    def write_message(signature, data)
      write signature + [data.bytesize].pack('N') + data
      return
    end
